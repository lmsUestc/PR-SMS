...
class ResNet(MammothBackbone):
    ...
    def __init__(
        ...
        if pretrained:
            ...
            ckpt = torch.load('/jmain02/home/J2AD016/jjw02/ffy12-jjw02/mammoth-master/backbone/resnet50-0676ba61.pth')
...
class MyResNet(MammothBackbone):
    ...
    def __init__(
        ...
        self.layer3, self.layer3_ = self._make_layer2(block, 256, layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1])
        self.layer4, self.layer4_ = self._make_layer2(block, 512, layers[3], stride=2,
                                       dilate=replace_stride_with_dilation[2])
    ...
    def _make_layer2(self, block: Bottleneck, planes: int, blocks: int,
                    stride: int = 1, dilate: bool = False) -> nn.Sequential:
        norm_layer = self._norm_layer
        downsample = None
        downsample2 = None
        ...
        layers.append(block(self.inplanes, planes, stride, downsample, self.groups,
                            self.base_width, previous_dilation, norm_layer))
        layers2.append(block(self.inplanes, planes, stride, downsample, self.groups,
                            self.base_width, previous_dilation, norm_layer))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, groups=self.groups,
                                base_width=self.base_width, dilation=self.dilation,
                                norm_layer=norm_layer))
            layers2.append(block(self.inplanes, planes, groups=self.groups,
                                base_width=self.base_width, dilation=self.dilation,
                                norm_layer=norm_layer))
    ...
    def GetAllFeaturs(self,inputs):
        out_0 = self.conv1(inputs)
        out_0 = self.bn1(out_0)
        if self.return_prerelu:
            out_0_t = out_0.clone()
        ...
        ####
        out_3_ = self.layer3_(out_2)
        out_4_ = self.layer4_(out_3_)

        feature = self.avgpool(out_4)
        feature = torch.flatten(feature, 1)

        feature_ = self.avgpool(out_4_)
        feature_ = torch.flatten(feature_, 1)

        return out_3,out_4,feature,out_3_,out_4_,feature_
    ...
    def forward(self, x: Tensor, returnt="out") -> Tensor:
        ...
        feature = torch.cat((feature,feature_),1)
        out = self.classifier(feature)
...
def myresnet50(num_classes: int, pretrained=False, **kwargs: Any) -> ResNet:
    r"""ResNet-50 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`_.
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return MyResNet(Bottleneck, [3, 4, 6, 3], num_classes=num_classes, pretrained=pretrained, **kwargs)
