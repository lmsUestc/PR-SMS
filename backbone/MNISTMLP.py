...
class MyMNISTMLP(MammothBackbone):
    """
    Network composed of two hidden layers, each containing 100 ReLU activations.
    Designed for the MNIST dataset.
    """
    ...
    def __init__(self, input_size: int, output_size: int) -> None:
        ...
        self.fc1_ = nn.Linear(self.input_size, 100)
        self.fc2_ = nn.Linear(100, 100)
        ...
        #features = torch.cat((self._features,self._features_), 1)

        self.classifier = nn.Linear(200, self.output_size)
        self.net = self.classifier#nn.Sequential(features, self.classifier)
        self.reset_parameters()
    ...
    def GetAllFeaturs(self, x: torch.Tensor):
        ...
        feats = self._features(x)
        feats2 = self._features_(x)
        features = torch.cat((feats, feats2), 1)
