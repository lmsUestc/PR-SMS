...
class BasicBlockDynamic(BasicBlock):
    """
...
class ResNetDynamic(ResNet):
    ...
    def __init__(self, block: BasicBlock, num_blocks: List[int],
        ...
        """
        super(ResNetDynamic, self).__init__(block, num_blocks, num_classes, nf)
        if old_cols is None:
            old_cols = []
    ...
    def forward(self, x: torch.Tensor, returnt='out') -> torch.Tensor:
        ...
        if len(self.old_cols) == 0:
            return super(ResNetDynamic, self).forward(x)
        else:
            with torch.no_grad():
...
def resnet18_pnn_2(nclasses: int, nf: int = 64,
    ...
    if old_cols is None:
        old_cols = []
    return ResNetDynamic(BasicBlockDynamic, [2, 2, 2, 2], nclasses, nf,
                     old_cols=old_cols, x_shape=x_shape)


def resnet18_pnn_3(nclasses: int, nf: int = 64,
                 old_cols: List[nn.Module] = None, x_shape: torch.Size = None):
    """
    Instantiates a ResNet18 network.
