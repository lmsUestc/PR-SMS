...
#from datasets.inception import InceptionV3

#block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[2048]

#ic_model = InceptionV3([block_idx]).to(device)
...
class MemoryCluster:
    ...
    def AddSupervisedSingleSample(self, x1, y1):

        dis1 = self.CalculateWDistance_Individual(x1, self.arr[self.alreadySampleIndex])
    ...
    def CalculateWDistance_DataBatch(self, dataset1, dataset2):
        sum1 = 0
        for i in range(np.shape(dataset1)[0]):
            d1 = self.CalculateWDistance_Individual(dataset1[i], dataset2[i])
            sum1 = sum1 + d1
        sum1 = sum1 / np.shape(dataset1)[0]
        return sum1
    ...
    def CalculateWDistance_Individual(self, x1, x2):
        if self.distance_type == "WDistance":
            ...
            # return score
            device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
            x1 = torch.tensor(x1).cuda().to(device=device, dtype=torch.float)
            x2 = torch.tensor(x2).cuda().to(device=device, dtype=torch.float)

            x1 = torch.reshape(x1, (-1, 3, self.input_size, self.input_size))
            x2 = torch.reshape(x2, (-1, 3, self.input_size, self.input_size))
            a1 = get_activations_byImages_WithoutModel2(ic_model, x1)
            a2 = get_activations_byImages_WithoutModel2(ic_model, x2)

            return np.mean(np.square(a1 - a2))
            '''
            #a1 = torch.tensor(a1).cuda().to(device=device, dtype=torch.float)
            #a2 = torch.tensor(a2).cuda().to(device=device, dtype=torch.float)

            #A = torch.reshape(a1, (-1))
            #B = torch.reshape(a2, (-1))
            A = a1
            B = a2
...
class MemoryClusterSupervised:
    def __init__(self, device, x, y,logits, maxSize, inputSize, TSF1, distance_type):
        self.device = device
        self.maxMemorySize = maxSize
        self.arr = []
        self.labelledArr = []
        ...
        for i in range(count):
            self.arr.append(x[i])
            self.labelledArr.append(y[i])
            self.logitArr.append(logits[i])
    ...
    def GetAllSamples(self):
        ...
        dataY = []
        logitArr = []
        #print(np.shape(self.labelledArr))
        #print(np.shape(self.logitArr))
        myLabel = self.labelledArr
        myLabel = np.reshape(myLabel,(np.shape(myLabel)[0],1))

        for j in range(np.shape(self.arr)[0]):
            if np.shape(dataX)[0] == 0:
                dataX = self.arr[j]
                dataX = np.reshape(dataX,(-1,3,32,32))
                dataY = myLabel[j]
                logitArr = self.logitArr[j]
                logitArr = np.reshape(logitArr,(-1,np.shape(self.logitArr)[1]))
            else:
                new1 = self.arr[j]
                new1 = np.reshape(new1,(-1,3,32,32))
                dataX = np.concatenate((dataX, new1), 0)
                dataY = np.concatenate((dataY, myLabel[j]), 0)
                new1logit = self.logitArr[j]
                new1logit = np.reshape(new1logit,(-1,np.shape(self.logitArr)[1]))
                logitArr = np.concatenate((logitArr, new1logit), 0)

        #print(np.shape(dataX))

        return dataX,dataY,logitArr
    ...
    def GetRandomSamples(self,n):
        ...
        for j in range(np.shape(self.arr)[0]):
            if np.shape(dataX)[0] == 0:
                dataX = self.arr[j]
                dataY = self.labelledArr[j]
                logitArr = self.logitArr[j]
            else:
                dataX = np.concatenate((dataX,self.arr[j]),0)
                dataY = np.concatenate((dataY,self.labelledArr[j]),0)
                logitArr = np.concatenate((logitArr, self.logitArr[j]),0)

        index = [i for i in range(len(dataX))]
        dataX = dataX[index]
        dataY = dataY[index]
        logitArr = logitArr[index]

        dataX = dataX[0:n]
        dataY = dataY[0:n]
        logitArr = logitArr[0:n]
        return dataX,dataY,logitArr
    ...
    def Calculate_CentrePoint(self):
        ...
        self.alreadyDistance = np.max(myarr2)
        self.alreadySample = self.arr[removedIndex]
        self.alreadySampleY = self.labelledArr[removedIndex]
        self.alreadyLogit = self.logitArr[removedIndex]
        self.alreadySampleIndex = removedIndex
    ...
    def AddSupervisedSingleSample(self, x1, y1,logit1):
        ...
        if self.CheckIsFull() == True:
            if dis1 > self.alreadyDistance:
                np.delete(self.arr, self.alreadySampleIndex)
                np.delete(self.labelledArr, self.alreadySampleIndex)
                np.delete(self.logitArr,self.alreadySampleIndex)
                # self.arr.remove(self.alreadySample)
                ...
                self.alreadySampleIndex = np.shape(self.arr)[0] - 1
                self.arr.append(x1)
                self.labelledArr.append(y1)
                self.logitArr.append(logit1)
        else:
            if dis1 > self.alreadyDistance or np.shape(self.arr)[0] == 1:
                self.alreadyDistance = dis1
                self.alreadySample = x1
    ...
    def CalculateWDistance_Individual(self, x1, x2):
        if self.distance_type == "WDistance":
            ...
            x1 = torch.reshape(x1, (-1, 3, 32, 32))
            x2 = torch.reshape(x2, (-1, 3, 32, 32))
...
######
class SelfGraphMemory2Framework:

    def __init__(self, name, device, input_size):

        self.maxSizeForEachMemory = 32  # 300
        ...
        self.maxRemainCount = 20
        self.isCheck = False
        self.modelExpansionThreshold = 1
    ...
    def GetRandomSamples(self,n):
        ...
        for j in range(np.shape(self.MemoryClusterArr)[0]):
            arr1 = self.MemoryClusterArr[j]
            x1,y1,logit1 = arr1.GetAllSamples()
            ...
            if np.shape(dataX)[0] == 0:
                ...
                dataX = np.concatenate((dataX,x1),0)
                dataY = np.concatenate((dataY,y1),0)
                logitArr = np.concatenate((logitArr,logit1))


        #print(np.shape(dataX))
        #print(np.shape(dataY))
        #print(np.shape(logitArr))
        ...
        dataY = np.reshape(dataY,np.shape(dataY)[0])

        dataX = dataX[0:n]
        dataY = dataY[0:n]
        logitArr = logitArr[0:n]

        dataX = torch.tensor(dataX).cuda().to(device=device, dtype=torch.float)
        dataY = torch.tensor(dataY).cuda().to(device=device, dtype=torch.int)
        logitArr = torch.tensor(logitArr).cuda().to(device=device, dtype=torch.float)

        return dataX,dataY,logitArr

    def MemoryBegin(self, x1, y1,logit):
        x1 = self.TransferTensorToNumpy(x1)
        y1 = self.TransferTensorToNumpy(y1)
        logit = self.TransferTensorToNumpy(logit)

        new1 = MemoryClusterSupervised(self.device, x1, y1,logit, self.maxSizeForEachMemory, self.input_size, self,
                                       self.distance_type)
        ...
        #print(np.shape(new1.arr))
        #print(np.shape(new1.labelledArr))
        #print(np.shape(new1.logitArr))

    def MemoryBeginUnsupervised(self, x1):
    ...
    def CheckModelExpansion(self, data):
        scoreArr = []

        allMemorizedSamples, _ = self.GiveMemorizedSamples()

        n_examples = np.shape(allMemorizedSamples)[0]
        index2 = [i for i in range(n_examples)]
        np.random.shuffle(index2)

        allMemorizedSamples = allMemorizedSamples[index2]
        allMemorizedSamples = allMemorizedSamples[0:np.shape(self.teacherArray[0].memoryBuffer)[0]]

        arr = []
        for i in range(np.shape(self.teacherArray)[0] - 1):
            teacher = self.teacherArray[i]

            # print("aaaa")
            # print(np.shape(allMemorizedSamples))
            # print(np.shape(teacher.memoryBuffer))

            d1 = self.MemoryClusterArr[0].CalculateWDistance_DataBatch(teacher.memoryBuffer, data)
            arr.append(d1)

        isExpansion = False
        minScore = np.min(arr)
        print("expansion score")
        print(minScore)
        if minScore > self.modelExpansionThreshold:
            # Build
            print("build")
            isExpansion = True

        return isExpansion, minScore

    def AddSupervisedDataBatch(self, x1, y1,logit1):
        x1 = self.TransferTensorToNumpy(x1)
        y1 = self.TransferTensorToNumpy(y1)
        logit1 = self.TransferTensorToNumpy(logit1)
        ...
        if self.GiveMemorySize() < self.maxMemorySize:
            ...
            for i in range(n):
                ...
                for j in range(np.shape(self.MemoryClusterArr)[0]):
                    ...
                    if self.distance_type == "WDistance":
                        dis1 = m1.CalculateWDistance_Individual(m1.centreSample, data1)
                    else:
                        dis1 = m1.CalculateWDistance_Individual(m1.centreSample, data1)
                ...
                minV = np.min(myArr)
                minvIndex = np.argmin(myArr)

                if minV >= self.MemoryClusterArr[0].maxMemorySize:
                    # Build a new memory cluster
                    arr = []
                    arr.append(data1)
                    arr2 = []
                    arr2.append(dataY)
                    arr3 = []
                    arr3.append(logit)
                    newMemory = MemoryClusterSupervised(self.device, arr, arr2,arr3, self.maxSizeForEachMemory,
                    ...
                    if self.GiveMemorySize() < self.maxMemorySize:
                        if self.MemoryClusterArr[minIndex].GiveCount() >= self.MemoryClusterArr[0].maxMemorySize:
                            self.MemoryClusterArr[minvIndex].AddSupervisedSingleSample(data1, dataY,logit)
                        else:
                            self.MemoryClusterArr[minIndex].AddSupervisedSingleSample(data1, dataY,logit)


        else:
            # update the remainCount
            for cc in range(np.shape(self.MemoryClusterArr)[0]):
                self.MemoryClusterArr[cc].remainCount = self.MemoryClusterArr[cc].remainCount + 1
            ...
            for i in range(n):
                ...
                if isBuild == True:
                    ...
                    m1 = self.MemoryClusterArr[i]
                    arr = []

                    for j in range(np.shape(self.MemoryClusterArr)[0]):
                        score1 = m1.CalculateWDistance_Individual(m1.centreSample, data1)
                        arr.append(score1)
                    ...
                    if minScore > self.threshold:
                        ...
                        self.MemoryClusterArr[minIndex] = newMemory
                        isBuild = True
                        print("Replace")
            '''
            ...
            for i in range(np.shape(self.MemoryClusterArr)[0]):
                ...
                for j in range(np.shape(self.MemoryClusterArr)[0]):
                    if i != j:
                        m2 = self.MemoryClusterArr[j]
                        score1 = m1.CalculateWDistance_Individual(m1.centreSample, m2.centreSample)
                        sum1 = sum1 + score1
                scoreArr.append(sum1)
            i = 0
            j = 0
            n = np.shape(x1)[0]
            isBuild = False
            ...
            for i in range(n):
                ...
                scoreArrCopy = deepcopy(scoreArr)
                sum1 = 0
                for j in range(np.shape(self.MemoryClusterArr)[0]):
                    m1 = self.MemoryClusterArr[j]
                    score1 = m1.CalculateWDistance_Individual(m1.centreSample, data1)
                    sum1 = sum1 + score1
                    scoreArrCopy[j] = scoreArrCopy[j] + score1

                scoreArrCopy.append(sum1)
                scoreArrCopy = np.array(scoreArrCopy)
                for jj in range(np.shape(scoreArrCopy)[0]):
                    scoreArrCopy[jj] = scoreArrCopy[jj] / np.shape(self.MemoryClusterArr)[0]

                minIndex = np.argmin(scoreArrCopy)
                minScore = np.min(scoreArrCopy)
                print(minScore)

                if self.isCheck:  # minScore > self.threshold:
                    if minScore > self.threshold:
                        if minIndex != (np.shape(scoreArrCopy)[0] - 1):
                            self.MemoryClusterArr[minIndex].remainCount = self.MemoryClusterArr[
                                                                              minIndex].remainCount + 1
                    ...
                    if minIndex != (np.shape(scoreArrCopy)[0] - 1):
                        self.MemoryClusterArr[minIndex].remainCount = self.MemoryClusterArr[minIndex].remainCount + 1
    ...
    def AddUnsupervisedDataBatch(self, x1):
        ...
        if self.GiveMemorySize() < self.maxMemorySize:
            ...
            for i in range(n):
                ...
                if minV >= self.MemoryClusterArr[0].maxMemorySize:
                    ...
                    arr = []
                    arr.append(data1)
                    newMemory = MemoryCluster(self.device, arr, self.maxSizeForEachMemory, self.input_size, self,
                                              self.distance_type)
                    self.MemoryClusterArr.append(newMemory)
                    self.memoryUnits = self.memoryUnits + 1
                    ...
                    if self.GiveMemorySize() < self.maxMemorySize:
                        if self.MemoryClusterArr[minIndex].GiveCount() >= self.MemoryClusterArr[0].maxMemorySize:
                            self.MemoryClusterArr[minvIndex].AddSingleSample(data1)
                        else:
                ...
                if self.isCheck:  # minScore > self.threshold:
                    if minScore > self.threshold:
                        if minIndex != (np.shape(scoreArrCopy)[0] - 1):
                            ...
                            newMemory = MemoryCluster(self.device, arr, self.maxSizeForEachMemory, self.input_size,
                                                      self,
                                                      self.distance_type)
    ...
    def AddUnsupervisedDataBatch_Files(self, x1):
        ...
        if self.GiveMemorySize() < self.maxMemorySize:
            ...
            for i in range(n):
                ...
                if minV >= self.MemoryClusterArr[0].maxMemorySize:
                    ...
                    if self.GiveMemorySize() < self.maxMemorySize:
                        if self.MemoryClusterArr[minIndex].GiveCount() >= self.MemoryClusterArr[0].maxMemorySize:
                            self.MemoryClusterArr[minvIndex].AddSingleSample_Files(data1)
                        else:
            ...
            for i in range(n):
                ...
                if isBuild == True:
                    ...
                    for j in range(np.shape(self.MemoryClusterArr)[0]):
                        score1 = m1.CalculateWDistance_Individual_Files(m1.centreSample, data1)
                        arr.append(score1)
            ...
            for i in range(np.shape(self.MemoryClusterArr)[0]):
                ...
                for j in range(np.shape(self.MemoryClusterArr)[0]):
                    if i != j:
                        m2 = self.MemoryClusterArr[j]
                        score1 = m1.CalculateWDistance_Individual_Files(m1.centreSample, m2.centreSample)
                        sum1 = sum1 + score1
                scoreArr.append(sum1)
            i = 0
            j = 0
    ...
    def GiveMemorizedSamples(self):
        ...
        for i in range(np.shape(self.MemoryClusterArr)[0]):
            ...
            oldY = self.MemoryClusterArr[i].labelledArr
            ...
            if np.shape(memory)[0] == 0:
                ...
                memoryY = np.concatenate((memoryY, oldY), 0)

        return memory, memoryY

    def GiveMemorizedSamplesUnsupervised(self):
        memory = []
        memoryY = []
        for i in range(np.shape(self.MemoryClusterArr)[0]):
            old = self.MemoryClusterArr[i].arr
