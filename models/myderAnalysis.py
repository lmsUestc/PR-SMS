...
class MyDerAnalysis(ContinualModel):
    NAME = 'myderAnalysis'
    ...
    def __init__(self, backbone, loss, args, transform):
        super(MyDerAnalysis, self).__init__(backbone, loss, args, transform)
    ...
    def end_task(self, dataset):
        ...
        if self.current_task == 0:
            self.firstTeacher = copy.deepcopy(self.net)

        '''
        if self.current_task > 0:
            if np.shape(self.slowArr)[0] < self.current_task:
                fastDistance = self.Estimate_FastRepresentation_Distance(self.lastInput,self.net,self.firstTeacher)
                slowDistance = self.Estimate_SlowRepresentation_Distance(self.lastInput,self.net,self.firstTeacher)
        ...
        myThirdName = "results/SplitCIFAR10_Slow_" + str(self.t1) + "_" + str(self.t2) + ".txt"
        ...
        myThirdName = "results/SplitCIFAR10_Fast_" + str(self.t1) + "_" + str(self.t2) + ".txt"
        # myThirdName = "results/Diffusion_Forgetting_RecoLoss_FirstTaskLearning.txt"
        f = open(myThirdName, "w", encoding="utf-8")
        for i in range(np.shape(arr1)[0]):
            f.writelines(arr1[i])
        ...
        arr1 = np.array(self.slowLossArr).astype('str')
        myThirdName = "results/SplitCIFAR10_SlowLoss_" + str(self.t1) + "_" + str(self.t2) + ".txt"
        # myThirdName = "results/Diffusion_Forgetting_RecoLoss_FirstTaskLearning.txt"
        f = open(myThirdName, "w", encoding="utf-8")
        for i in range(np.shape(arr1)[0]):
            f.writelines(arr1[i])
        ...
        arr1 = np.array(self.fastLossArr).astype('str')
        myThirdName = "results/SplitCIFAR10_FastLoss_" + str(self.t1) + "_" + str(self.t2) + ".txt"
        # myThirdName = "results/Diffusion_Forgetting_RecoLoss_FirstTaskLearning.txt"
        f = open(myThirdName, "w", encoding="utf-8")
        for i in range(np.shape(arr1)[0]):
            f.writelines(arr1[i])
    ...
    def Calculate_Slow_Loss(self,inputs,labels):
        Oldout1, Oldout2, Oldout3, Oldout4, Oldfeature, Oldout1_, Oldout2_, Oldout3_, Oldout4_, Oldfeature_ = self.old_net.GetAllFeaturs(
            inputs)
    ...
    def Estimate_FastRepresentation_Distance(self,inputs,student,teacher):
        with torch.no_grad():
            Oldout1, Oldout2, Oldout3, Oldout4, Oldfeature,Oldout1_, Oldout2_, Oldout3_, Oldout4_, Oldfeature_ = teacher.GetAllFeaturs(inputs)
            out1_, out2_, out3_, out4_, feature_,out1__, out2__, out3__, out4__, feature__ = student.GetAllFeaturs(inputs)
            ...
            d3 = HSIC(out3, out3_)
            d4 = HSIC(out4, out4_)
            d5 = HSIC(Oldfeature_, feature_)

            d = d1 + d2 + d3 + d4 + d5

        return d

    def Estimate_SlowRepresentation_Distance(self,inputs,student,teacher):
        with torch.no_grad():
            Oldout1, Oldout2, Oldout3, Oldout4, Oldfeature,Oldout1_, Oldout2_, Oldout3_, Oldout4_, Oldfeature_ = teacher.GetAllFeaturs(inputs)
            out1_, out2_, out3_, out4_, feature_,out1__, out2__, out3__, out4__, feature__ = student.GetAllFeaturs(inputs)
    ...
    def observe(self, inputs, labels, not_aug_inputs, epoch=None):
        ...
        if self.current_task > 0:
            ...
            if not self.buffer.is_empty():
                ...
                self.fastLossArr.append(r_disentangled_loss.item())
                self.slowLossArr.append(r_loss.item())
        ...
        if self.current_task > 0:
            fastDistance = self.Estimate_FastRepresentation_Distance(inputs,self.net,self.firstTeacher)
            slowDistance = self.Estimate_SlowRepresentation_Distance(inputs,self.net,self.firstTeacher)
            fastDistance = fastDistance.detach().cpu().numpy()
            slowDistance = slowDistance.detach().cpu().numpy()
            self.slowArr.append(slowDistance)
            self.fastArr.append(fastDistance)
