...
import copy
from models.utils.MMDCriterion import *


class MyDerMMD(ContinualModel):
    NAME = 'mydermmd'
    ...
    def __init__(self, backbone, loss, args, transform):
        super(MyDerMMD, self).__init__(backbone, loss, args, transform)
    ...
    def Calculate_Distance(self, inputs, labels):
        ...
        d1 = self.mmdCriterion.rbf_mmd(out1, out1_)
        d2 = self.mmdCriterion.rbf_mmd(out2, out2_)
        d3 = self.mmdCriterion.rbf_mmd(out3, out3_)
        d4 = self.mmdCriterion.rbf_mmd(out4, out4_)
        d5 = self.mmdCriterion.rbf_mmd(Oldfeature, feature_)

        d = d1 + d2 + d3 + d4 + d5

        # Calculate distance between the common z and specific z
        dd1 = self.mmdCriterion.rbf_mmd(out1_, out1__)
        dd2 = self.mmdCriterion.rbf_mmd(out2_, out2__)
        dd3 = self.mmdCriterion.rbf_mmd(out3_, out3__)
        dd4 = self.mmdCriterion.rbf_mmd(out4_, out4__)
        dd5 = self.mmdCriterion.rbf_mmd(feature_, feature__)
    ...
    def Estimate_FastRepresentation_Distance(self, inputs):
        with torch.no_grad():
            Oldout1, Oldout2, Oldout3, Oldout4, Oldfeature, Oldout1_, Oldout2_, Oldout3_, Oldout4_, Oldfeature_ = self.old_net.GetAllFeaturs(
                inputs)
            out1_, out2_, out3_, out4_, feature_, out1__, out2__, out3__, out4__, feature__ = self.net.GetAllFeaturs(
                inputs)
    ...
    def observe(self, inputs, labels, not_aug_inputs, epoch=None):
        ...
        if self.current_task > 0:
            ...
            if not self.buffer.is_empty():
                ...
                #Add a threshold to avoid loss nan
                threshold = 10
                if - r_disentangled_loss * t2 > threshold:
                    rr = r_loss * t1
                else:
