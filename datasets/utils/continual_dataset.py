...
def getdataload(train_dataset: Dataset, test_dataset: Dataset,
                         setting: ContinualDataset,i) -> Tuple[DataLoader, DataLoader]:
    ...
    if setting.SETTING == 'class-il' or setting.SETTING == 'task-il':
        train_mask = np.logical_and(np.array(train_dataset.targets) >= i,
                                    np.array(train_dataset.targets) < i + setting.N_CLASSES_PER_TASK)
        test_mask = np.logical_and(np.array(test_dataset.targets) >= i,
                                   np.array(test_dataset.targets) < i + setting.N_CLASSES_PER_TASK)
...
def store_masked_loaders(train_dataset: Dataset, test_dataset: Dataset,
    ...
    if setting.SETTING == 'class-il' or setting.SETTING == 'task-il':
        print('setting:',np.array(train_dataset.targets).shape,setting.i,setting.N_CLASSES_PER_TASK)
        if type(setting.N_CLASSES_PER_TASK) == list:
            classes_per_task = setting.N_CLASSES_PER_TASK[setting.i // 20]
            print('setting2:',np.array(train_dataset.targets).shape,setting.i,setting.N_CLASSES_PER_TASK,classes_per_task)
            train_mask = np.logical_and(np.array(train_dataset.targets) >= setting.i,
                                    np.array(train_dataset.targets) < setting.i + classes_per_task)
            test_mask = np.logical_and(np.array(test_dataset.targets) >= setting.i,
                                   np.array(test_dataset.targets) < setting.i + classes_per_task)
        else:
            train_mask = np.logical_and(np.array(train_dataset.targets) >= setting.i,
    ...
    if setting.SETTING == 'task-il' or setting.SETTING == 'class-il':
        if type(setting.N_CLASSES_PER_TASK) == list:
            setting.i += setting.N_CLASSES_PER_TASK[setting.i // 20]
        else:
            setting.i += setting.N_CLASSES_PER_TASK
        setting.c_task += 1
    return train_loader, test_loader
