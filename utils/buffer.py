...
from backbone.MMDLoss_ import MMSD
...
class FeaBuffer:
    ...
    def __init__(self, buffer_size, device="cpu"):
        ...
        self.MMDLoss = MMSD().to(self.device)
        # self.classList = [0] * numofclass
    ...
    def ResizeBufferSize(self, size):
        self.buffer_size = size
        # randomly remove data samples from the memory buffer

        arr1 = []
        for c1 in range(np.shape(self.examples)[0]):
            arr1.append(c1)
        arr1 = np.array(arr1)

        selectedIndex = np.random.choice(arr1, size=self.buffer_size, replace=False)
        ...
        print("ppp")
        print(np.shape(self.examples))
        print("mmm")
        print(np.shape(self.labels))
        print("ttt")
        print(np.shape(self.logits))

        for i in range(np.shape(self.examples)[0]):
            isTrue = False
            for j in range(np.shape(selectedIndex)[0]):
                if i != selectedIndex[j]:
                    isTrue = True
                    break
            if isTrue == True:
                x1 = torch.reshape(self.examples[i], (1, 3, 32, 32))
                label1 = torch.reshape(self.labels[i], (1, 1))
                logit1 = torch.reshape(self.logits[i], (1, np.shape(self.logits[i])[0]))
                if np.shape(newExamples)[0] == 0:
                    newExamples = x1
                else:
                    newExamples = torch.cat((newExamples, x1), 0)

                if np.shape(newLabels)[0] == 0:
                    newLabels = label1
                else:
                    newLabels = torch.cat((newLabels, label1), 0)

                if np.shape(newLogits)[0] == 0:
                    newLogits = logit1
                else:
                    newLogits = torch.cat((newLogits, logit1), 0)

        self.examples = newExamples
        self.labels = newLabels
        self.logits = newLogits
    ...
    def add_data_new(self, examples, labels=None, logits=None, task_labels=None, attention_maps=None):
        """
        ...
        for i in range(examples.shape[0]):
            ...
            a1 = examples[i].to(self.device)
            a1 = torch.reshape(a1, (1, 3, 32, 32))
            self.examples = torch.cat((self.examples, a1), 0)
            # self.examples.append(examples[i].to(self.device))
            if labels is not None:
                a2 = labels[i].to(self.device)
                a2 = torch.reshape(a2, (1, np.shape(a2)[0]))
                self.labels = torch.cat((self.labels, a2), 0)
                # self.labels.append(labels[i].to(self.device))
            if logits is not None:
                a3 = logits[i].to(self.device)
                a3 = torch.reshape(a3, (1, np.shape(a3)[0]))
                self.logits = torch.cat((self.logits, a3), 0)
    ...
    def add_data(self, examples, labels=None, logits=None, task_labels=None, attention_maps=None):
        ...
        for i in range(examples.shape[0]):
            ...
            # print('index:', index, i, examples.shape, examples[i].shape)
            self.num_seen_examples += 1
            if index >= 0:
        ...
        # print("exam:", self.examples.shape, self.labels.shape, self.num_seen_examples, self.logits.shape)

    def add_Singledata(self, examples, labels=None, logits=None, task_labels=None, attention_maps=None):
        """
        Adds the data to the memory buffer according to the reservoir strategy.
    ...
    def get_data(self, size: int, transform: nn.Module = None, return_index=False, device=None, mask_task_out=None,
                 cpt=None) -> Tuple:
        """
        ...
        if mask_task_out is not None:
            ...
            samples_mask = (self.task_labels != mask_task_out) if hasattr(self,
                                                                          'task_labels') else self.labels // cpt != mask_task_out

        num_avail_samples = self.examples.shape[0] if mask_task_out is None else samples_mask.sum().item()
        # print('num:', num_avail_samples, self.examples.shape)
        num_avail_samples = min(self.num_seen_examples, num_avail_samples)
        # print('num2:', num_avail_samples)
        ...
        for attr_str in self.attributes[1:]:
            ...
            if hasattr(self, attr_str):
                ...
                # print("re:", len(ret_tuple), ret_tuple[-1])
    ...
    def filter_by_kmeans(self, tempbuffer, final_buffer_size):
        examples = tempbuffer.examples[:tempbuffer.num_seen_examples] # [n, 768]
        labels = tempbuffer.labels[:tempbuffer.num_seen_examples]     # [n]
        logits = tempbuffer.logits[:tempbuffer.num_seen_examples]     # [n]
    
        unique_labels = labels.unique()  
        num_classes = len(unique_labels)  

        samples_per_class = final_buffer_size // num_classes
        ...
        for i in range(num_classes):
            ...
            class_data = examples[class_indices]

            kmeans = KMeans(n_clusters=1, random_state=0).fit(class_data.cpu().numpy())
            center = torch.tensor(kmeans.cluster_centers_[0], dtype=torch.float32).to(examples.device)

            distances = torch.norm(class_data - center, dim=1)

            sorted_indices = torch.argsort(distances, descending=True)

            if i < num_classes - 1:
                top_indices = sorted_indices[:samples_per_class]
            elif i == num_classes - 1:
                samples_index = final_buffer_size - samples_per_class * i
                top_indices = sorted_indices[:samples_index]

            selected_examples.append(class_data[top_indices])
            selected_labels.append(labels[class_indices][top_indices])
            selected_logits.append(logits[class_indices][top_indices])
            

        final_examples = torch.cat(selected_examples, dim=0)
        final_labels = torch.cat(selected_labels, dim=0)
        final_logits = torch.cat(selected_logits, dim=0)
        for i in range(final_examples.shape[0]):
            # print("i:", i)
            self.num_seen_examples += 1
            if i >= 0:
                self.examples[i] = final_examples[i].to(self.device)
                if labels is not None:
                    self.labels[i] = final_labels[i].to(self.device)
                if logits is not None:
                    self.logits[i] = final_logits[i].to(self.device)

    def interpolate_and_append(self, k):
        n = self.num_seen_examples
        examples = self.examples[:self.num_seen_examples] # [n, 768]
        labels = self.labels[:self.num_seen_examples]     # [n]
        logits = self.logits[:self.num_seen_examples]     # [n]
        ...
        unique_labels = labels.unique()
        numofclass = len(unique_labels)

        for i in range(numofclass):
            label = unique_labels[i]
            if i < numofclass - 1:
                numofsamples = k // numofclass
            elif i == numofclass - 1:
                numofsamples = k - k // numofclass * i
            
            class_indices = (labels == label).nonzero(as_tuple=True)[0]
            class_features = examples[class_indices]
            class_logits = logits[class_indices]

            for _ in range(numofsamples):
                idx1, idx2 = torch.randint(0, len(class_indices), (2,))
                weight = torch.rand(1).item()

                interpolated_feature = (1 - weight) * class_features[idx1] + weight * class_features[idx2]
                interpolated_logits = (1 - weight) * class_logits[idx1] + weight * class_logits[idx2]

                new_features.append(interpolated_feature)
                new_labels.append(label)
                new_logits.append(interpolated_logits)

        if new_features:
            new_features = torch.stack(new_features)  # 形状 [k, 768]
            new_labels = torch.stack(new_labels)      # 形状 [k]
            new_logits = torch.stack(new_logits)      # 形状 [k, m]

        for i in range(new_features.shape[0]):
            self.num_seen_examples += 1
            if i >= 0:
                self.examples[i + n] = new_features[i].to(self.device)
                if labels is not None:
                    self.labels[i + n] = new_labels[i].to(self.device)
                if logits is not None:
                    self.logits[i + n] = new_logits[i].to(self.device)

    def interpolate_grad(self, model, k, alpha, beta, epsilon=0.1, use_gradient=False):
        """
        对特征进行插值生成 k 个新特征，并追加到原始数据中。
        可选地使用梯度引导的扰动生成更难的样本。
    
        Args:
            k: int, 要生成的新特征数量。
            epsilon: float, 梯度扰动强度（默认 0.1）。
            use_gradient: bool, 是否使用梯度引导扰动（默认 False）。
    
        Returns:
            None
        """
        n = self.num_seen_examples
        examples = self.examples[:n]  # [n, 768]
        labels = self.labels[:n]      # [n]
        logits = self.logits[:n]      # [n, num_classes]
        ...
        for i in range(num_classes):
            ...
            num_samples = k // num_classes if i < num_classes - 1 else k - (k // num_classes) * i
            ...
            for _ in range(num_samples):
                ...
                if use_gradient:
                    interpolated_feature.requires_grad = True
                    
                    logits_pred, t_fea = model.net(interpolated_feature.unsqueeze(0))  
                    logits_pred = logits_pred.to(self.device)
                    label = label.to(self.device)
                    loss_ce = beta * model.loss(logits_pred, label.unsqueeze(0))

                    for param in model.net.classifier.parameters():
                        param.requires_grad = False
                    for param in model.net.fc.parameters():
                        param.requires_grad = False
                    loss_ce.backward()
                    grad = interpolated_feature.grad
                    
                    perturbation = epsilon * grad.sign()
                    interpolated_feature = interpolated_feature + perturbation
                    interpolated_feature = interpolated_feature.detach()  
        ...
        if new_features:
            new_features = torch.stack(new_features)  # 形状 [k, 768]
            new_labels = torch.stack(new_labels)      # 形状 [k]
            new_logits = torch.stack(new_logits)      # 形状 [k, num_classes]
    ...
    def select_center_subset(self, tempbuffer, k, subset_size=10, q=10):
        n = tempbuffer.num_seen_examples
        examples = tempbuffer.examples[:tempbuffer.num_seen_examples] # [n, 768]
        labels = tempbuffer.labels[:tempbuffer.num_seen_examples]     # [n]
        logits = tempbuffer.logits[:tempbuffer.num_seen_examples]     # [n]
        feature_dim = examples.shape[1]
        assert n >= subset_size, "Buffer 中样本数量不足！"

        
        subsets = []
        for _ in range(k):
            indices = torch.randint(0, n, (subset_size,))
            subsets.append(examples[indices])

        
        distances = torch.zeros(k, k)
        for i in range(k):
            for j in range(i + 1, k):
                subset_i = subsets[i].mean(dim=0).cpu().numpy()
                subset_j = subsets[j].mean(dim=0).cpu().numpy()
                distance = wasserstein_distance(subset_i, subset_j)
                distances[i, j] = distance
                distances[j, i] = distance

        mean_distances = distances.sum(dim=1)
        center_index = torch.argmin(mean_distances)
        center_subset = subsets[center_index]

        center_distances = torch.cdist(center_subset, center_subset)
        center_point_index = torch.argmin(center_distances.sum(dim=1))
        center_point = center_subset[center_point_index]

        all_distances = torch.norm(examples - center_point, dim=1)
        top_q_indices = torch.argsort(all_distances)[:q]

        final_examples = examples[top_q_indices]
        final_labels = labels[top_q_indices]
        final_logits = logits[top_q_indices]
        
        self.examples = final_examples
        self.labels = final_labels
        self.logits = final_logits
        self.num_seen_examples = q
        self.buffer_size = q

    def select_center_subset_within_class(self, tempbuffer, k, m=10, q=10):
        n = tempbuffer.num_seen_examples
        examples = tempbuffer.examples[:n]  # [n, 768]
        labels = tempbuffer.labels[:n]  # [n]
        logits = tempbuffer.logits[:n]  # [n]
    
        unique_labels = torch.unique(labels)
        final_examples = []
        final_labels = []
        final_logits = []
        num = q // len(unique_labels)
    
        # 遍历每个类别
        for label in unique_labels:
            # 获取当前类别的样本
            class_indices = (labels == label).nonzero(as_tuple=True)[0]
            class_examples = examples[class_indices]
            class_labels = labels[class_indices]
        
            subsets = []
            for _ in range(k):
                subset_indices = torch.randint(0, len(class_examples), (m,))
                subsets.append(class_examples[subset_indices])
            ...
            all_distances = torch.norm(class_examples - center_point, dim=1)
            top_q_indices = torch.argsort(all_distances)[:num]
        
            final_examples.append(class_examples[top_q_indices])
            final_labels.append(class_labels[top_q_indices])
            final_logits.append(logits[class_indices][top_q_indices])

        self.examples = torch.cat(final_examples, dim=0)
        self.labels = torch.cat(final_labels, dim=0)
        self.logits = torch.cat(final_logits, dim=0)
        self.num_seen_examples = q   # 更新已见样本数量
        self.buffer_size = q   
